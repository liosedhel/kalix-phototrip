...
class PlaceByMapIdViewImpl(context: ViewContext) extends AbstractPlaceByMapIdView {
  ...
  override def processPlaceCreated(state: CurrentPlace, placeCreated: PlaceCreated): UpdateEffect[CurrentPlace] = {
    effects.updateState(
      CurrentPlace(placeCreated.placeId, placeCreated.mapId, placeCreated.description, placeCreated.coordinates)
    )
  }

  override def processPhotoLinkAdded(state: CurrentPlace, photoLinkAdded: PhotoLinkAdded): UpdateEffect[CurrentPlace] =
