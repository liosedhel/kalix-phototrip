...
class WorldMapValueEntity(context: ValueEntityContext) extends AbstractWorldMapValueEntity {
  ...
  override def get(currentState: WorldMapState, getWorldMap: api.GetWorldMap): ValueEntity.Effect[api.CurrentWorldMap] = {
  ...
  private def isInitialized(currentState: WorldMapState): Boolean = {
    currentState != emptyState
  }

  private def isValid(createWorldMap: api.CreateWorldMap): Boolean = {
  ...
  private def toWorldMap(createWorldMap: api.CreateWorldMap): WorldMapState = {
    WorldMapState(mapId = createWorldMap.mapId, creatorId = createWorldMap.creatorId, description = createWorldMap.description)
  }
